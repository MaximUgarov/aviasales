...
const List: FC<ListProps> = ({ isLoading, items }: ListProps) => {
    ...
    return (
        <Fragment>
            {isLoading ? items.slice(0, listLength).map((i, index) => <Item item={i} key={index + 1} />) : <><SkeletonItem quantity={listLength} /> <Spinner size='100px' thickness='5px' color='#008EFF' animationSpeed="2s" background={true} /></>}
